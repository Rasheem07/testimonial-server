...
export const fetchTestimonialspaceData = async (space_name: string) => {
  const client = await PostgresClient.connect();
  try {
    // Check Redis cache first
    const cachedData = await redisClient.get(space_name);
    if (cachedData) {
    ...
    await redisClient.set(space_name, JSON.stringify(result.rows[0]));
